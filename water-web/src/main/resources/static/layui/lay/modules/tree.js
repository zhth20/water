...
layui.define('jquery', function (exports) {
    ...
    Tree.prototype.getChecked = function (key) {
        var checked = [];
        var get = function (items) {
            layui.each(items, function (index, item) {
                if (item['checked']) checked.push(item[key]);
                if (item.children) get(item.children);
            });
        }
        get(this.options.nodes);
        return checked;
    }
