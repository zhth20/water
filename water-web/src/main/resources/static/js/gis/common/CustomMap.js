layui.define(['jquery', 'layer', 'CustomControl', 'CustomMarkers'], function (exports) {
    var $ = layui.jquery,
        layer = layui.layer,
        CustomControl = layui.CustomControl,
        CustomMarkers = layui.CustomMarkers;
     ...
     * @constructor
     */
    function CustomMap(id, customer) {
        var self = this;
        ...
        this.createMap();
        this.initData().done(function (data) {
    ...
    CustomMap.prototype.initData = function () {
        var self = this;
        /*
         var url = $('#ctx').val() + '/gis/map/map.do';
         $.post(url, {id:self.customer.id}, {dataType:'json'}, function (data) {
        ...
        return $.ajax({
            ...
            layer.msg('获取地图数据失败', {icon: 2});
        });
    }
    CustomMap.prototype.initialize = function (data) {

        this.marker = new MapMarker(this);

        this.customPipes = new CustomPipes(data, this);

        this.control = new MapControl(this);
    ...
    CustomMap.prototype.initArea = function () {
        ...
        this.polygon = new BMap.Polygon(area, {
            fillColor: "#96D3FD",
            strokeColor: "#85B4D5",
            fillOpacity: 0.2,
            strokeWeight: 5,
